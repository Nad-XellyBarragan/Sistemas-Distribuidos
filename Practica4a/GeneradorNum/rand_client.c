...
static void rand_prog_1(const char *host, long seed, int iters)
{
    ...
    // INICIALIZA_RANDOM espera un puntero a long
    void *result = inicializa_random_1(&seed, clnt);
    if (result == NULL) {
    ...
    // OBTIENE_SIGUIENTE_RANDOM: aunque es void, necesita un argumento dummy
    char dummy = 0;
    for (int i = 0; i < iters; i++) {
        double *r = obtiene_siguiente_random_1((void*)&dummy, clnt);
        if (!r) {
